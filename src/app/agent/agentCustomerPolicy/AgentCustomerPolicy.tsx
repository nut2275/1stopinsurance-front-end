...
import { Search, Edit, X, Save, Upload, Eye, ChevronLeft, ChevronRight, FileText, CreditCard, Calendar, Copy, Check, AlertCircle, Download, Filter, User } from "lucide-react";
...
export default function AgentPolicyPage() {
  ...
  // 🟢 แก้ไข: ฟังก์ชัน fetchData ให้ดึง Token มาใส่ Header
  const fetchData = async () => {
    try {
      ...
      if (!token) {
        // ถ้าไม่มี Token ให้หยุดทำงาน หรือ redirect ไป login
        console.error("No token found");
        return;
      }

      // แนบ Token ไปใน headers
      const res = await api.get("/purchase/agent/my-history", {
        headers: {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1); 
  }, [searchName, searchCompany, searchPolicyNo, searchStatus, sortOrder, filterCarBrand, filterCarModel, filterCarSubModel, purchases]);
  ...
  // 🟢 แก้ไข: handleSave ให้ส่ง Token ด้วยเช่นกัน
  const handleSave = async () => { 
      if (!selectedItem) return; 
      try { 
          const token = localStorage.getItem("token");
          await api.put(`/purchase/agent/${selectedItem._id}`, { ...editForm }, {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      ...
      <h1 className="text-2xl font-bold mb-6 text-gray-800">รายการงานขายของฉัน (My Policies)</h1>
      ...
      <div className="bg-white p-5 rounded-lg shadow-md mb-6 space-y-4">
         <div className="flex items-center gap-2 text-gray-700 font-semibold border-b pb-2 mb-2">
             <Filter className="w-4 h-4"/> ค้นหาและกรองข้อมูล
         </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex justify-center items-start md:items-center z-50 p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-4xl rounded-xl shadow-2xl animate-fade-in my-8 relative flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-6 border-b sticky top-0 bg-white z-10 rounded-t-xl">
              <div><h2 className="text-xl font-bold text-gray-800">รายละเอียดงานขาย</h2><p className="text-sm text-gray-500">ID: {selectedItem._id}</p></div>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 overflow-y-auto flex-grow">
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                      <h3 className="font-semibold text-gray-800 flex items-center gap-2 pb-2 border-b"><User className="w-4 h-4 text-blue-600"/> ข้อมูลลูกค้า (Read Only)</h3>
                      <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="text-xs text-gray-500">ชื่อจริง</label>
                            <input type="text" className="w-full border rounded p-1.5 text-sm outline-none bg-gray-100 text-gray-500 cursor-not-allowed" value={editForm.customer_first_name} readOnly disabled />
                          </div>
                          <div>
                            <label className="text-xs text-gray-500">นามสกุล</label>
                            <input type="text" className="w-full border rounded p-1.5 text-sm outline-none bg-gray-100 text-gray-500 cursor-not-allowed" value={editForm.customer_last_name} readOnly disabled />
                          </div>
                      </div>

                      <h3 className="font-semibold text-gray-800 flex items-center gap-2 pb-2 border-b pt-2"><Edit className="w-4 h-4 text-blue-600"/> ข้อมูลประกันและรถยนต์</h3>
                      <div className="grid grid-cols-2 gap-2">
